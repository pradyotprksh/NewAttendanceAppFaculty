...
public class NotificationActivity extends AppCompatActivity {
    ...
    private String dataMessage, dataFrom, dataDesignation, name, image, messageOnValue, notificationId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        starBtn.setOnLikeListener(new OnLikeListener() {
            ...
            public void liked(LikeButton likeButton) {
                ...
                mFirestore1.collection("Faculty").document(user_id).collection("Notifications").document(notificationId).update(likedNotification).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
    ...
    protected void onStart() {
        ...
        try {
            mFirestore1.collection("Faculty").document(user_id).collection("Notifications").document(notificationId).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                @Override
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if (task.isSuccessful()) {
                        if (task.getResult().exists()) {
